...
namespace MathParser
{
	...
	public class Parser
	{
		...
		{
			// Solve Sin, Cos, Tan...
			Regex regEx = new Regex( @"([a-z]{2,})([\+-]?\d+"+dec+@"*\d*[eE][\+-]*\d+|[\+-]?\d+"+dec+@"*\d*)", RegexOptions.IgnoreCase );
			Match m = regEx.Match( Expression );
			...
			regEx = null;
			regEx = new Regex( @"(\d+"+dec+@"*\d*[eE][\+-]?\d+|\d+"+dec+@"*\d*)!" ); // Search for patterns like 5!
			m = regEx.Match( Expression );
			while( m.Success )
			{
				double n = Convert.ToDouble( m.Groups[1].Value );
			...
			regEx = null;
			// Solve power.
			regEx = new Regex( @"\{(.+?)\}\^(-?\d+"+dec+@"*\d*[eE][\+-]?\d+|-?\d+"+dec+@"*\d*)" ); // Search for patterns like {-5}^-1
			...
			regEx = null;
			regEx = new Regex( @"(\d+"+dec+@"*\d*e[\+-]?\d+|\d+"+dec+@"*\d*)\^(-?\d+"+dec+@"*\d*[eE][\+-]?\d+|-?\d+"+dec+@"*\d*)" ); // Search for patterns like 5^-1
			m = regEx.Match( Expression, 0 );
			while( m.Success )
			{
			...
			regEx = new Regex( @"([\+-]?\d+"+dec+@"*\d*[eE][\+-]?\d+|[\-\+]?\d+"+dec+@"*\d*)([\/\*])(-?\d+"+dec+@"*\d*[eE][\+-]?\d+|-?\d+"+dec+@"*\d*)" );
			m = regEx.Match( Expression, 0 );
			while( m.Success )
			{
				double result;
